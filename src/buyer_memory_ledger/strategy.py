...
REPO = 'buyer-memory-ledger'
PROJECT_TERMS = ['ai-native', 'depends', 'remembering', 'buyer', 'preferences', 'structures', 'owner', 'constraints']
PROJECT_METRICS = ['ai-native_coverage', 'depends_risk', 'remembering_precision', 'buyer_latency']
PROJECT_FAILURES = ['ai-native_drift', 'depends_gap', 'remembering_misroute', 'buyer_blindspot']
PROJECT_ARCHETYPES = [{'name': 'ai-native evidence replay', 'trigger': 'ai-native signal changes while depends context is stale', 'expected': 'block release until cited evidence is regenerated'}, {'name': 'depends boundary probe', 'trigger': 'depends handoff crosses a policy or trust boundary', 'expected': 'route to reviewer with evidence packet'}, {'name': 'remembering regression harness', 'trigger': 'remembering behavior regresses against the last accepted fixture', 'expected': 'open a regression issue with trace and benchmark delta'}, {'name': 'buyer operator packet', 'trigger': 'buyer output needs a human-readable audit packet', 'expected': 'accept only if decision claims cite fixture evidence'}]
PROJECT_DIRECTION = 'A buyer-preference memory and explainability ledger that replays synthetic sell-side mandates against acquirer criteria, explains why buyers were ranked, and detects stale or contradictory buyer memory.'
...
HERO_TITLE = 'DealFlowAgent Adversarial Replay Board'
